...
static const char* getStallReasonString(
  ...
  switch (record->kind) {
    case CUPTI_ACTIVITY_KIND_SOURCE_LOCATOR: {
      ...
      printf("Source Locator Id %d, File %s Line %d\n", sourceLocator->id,
             sourceLocator->fileName, sourceLocator->lineNumber);
      break;
    }
    case CUPTI_ACTIVITY_KIND_PC_SAMPLING: {
      ...
      printf("source %u, functionId %u, pc 0x%llx, corr %u, samples %u",
             psRecord->sourceLocatorId, psRecord->functionId,
             (unsigned long long)psRecord->pcOffset, psRecord->correlationId,
             psRecord->samples);

      // latencySamples Field is valid for devices with compute capability 6.0
      // and higher.
      if (prop.major >= 6) {
    ...
    case CUPTI_ACTIVITY_KIND_PC_SAMPLING_RECORD_INFO: {
      ...
      printf(
          "corr %u, totalSamples %llu, droppedSamples %llu, "
          "samplingPeriodInCycles %llu\n",
    ...
    case CUPTI_ACTIVITY_KIND_FUNCTION: {
      ...
      printf("id %u, ctx %u, moduleId %u, functionIndex %u, name %s\n",
             fResult->id, fResult->contextId, fResult->moduleId,
             fResult->functionIndex, fResult->name);
      break;
    }
    default:
      printf("unknown\n");
...
void finiTrace() { CUPTI_CALL(cuptiActivityFlushAll(0)); }
...
static void do_pass(cudaStream_t stream) {
  ...
  // configure api needs to be called before activity enable for chips till
  // Pascal and for Volta+ order does not matter
  ...
  VecAdd<<<blocksPerGrid, THREADS_PER_BLOCK, 0, stream>>>(d_A, d_B, d_C,
                                                          ARRAY_SIZE);
...
int main(int argc, char* argv[]) {
  ...
  if (!((prop.major > 5) || ((prop.major == 5) && (prop.minor == 2)))) {
    printf(
        "sample is waived on this device, pc sampling is supported on devices "
        "with compute capability 5.2 or 6.0 and higher\n");
    return 0;
  }
