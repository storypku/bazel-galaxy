...
//   -persistent{0,1,2} : Specify recurrence algorithm (standard, persist
//   dynamic, persist static) -P{s,d,h}          : Specify data type precision
//   (float, double, half) -H                 : Display this help message
//
// Reference outputs (calculated on an M40 GPU)
// golden_1.txt (default case if you just run ./RNN)
// > ./RNN -seqLength20 -numLayers2 -hiddenSize512 -inputSize512 -miniBatch64
// -mode0 Forward: 1250 GFLOPS Backward: 1896 GFLOPS, (1299 GFLOPS), (3511
// GFLOPS) y checksum 1.315793E+06     hy checksum 1.315212E+05 dx
// checksum 6.676003E+01    dhx checksum 6.425050E+01 dw checksum 1.453750E+09
//
// golden_2.txt
// > ./RNN -seqLength20 -numLayers2 -inputSize512 -hiddenSize512 -miniBatch64
// -mode1 Forward: 1225 GFLOPS Backward: 1910 GFLOPS, (1299 GFLOPS), (3601
// GFLOPS) y checksum 6.319591E+05     hy checksum 6.319605E+04 dx
// checksum 4.501830E+00    dhx checksum 4.489543E+00 dw checksum 5.012598E+07
//
// golden_3.txt
// > ./RNN -seqLength20 -numLayers2 -inputSize512 -hiddenSize512 -miniBatch64
// -mode2 Forward: 2569 GFLOPS Backward: 2654 GFLOPS, (2071 GFLOPS), (3694
// GFLOPS) y checksum 5.749536E+05     cy checksum 4.365091E+05     hy
// checksum 5.774818E+04 dx checksum 3.842206E+02    dcx checksum 9.323785E+03
// dhx checksum 1.182562E+01 dw checksum 4.313461E+08
//
// golden_4.txt
// > ./RNN -seqLength20 -numLayers2 -inputSize512 -hiddenSize512 -miniBatch64
// -mode3 Forward: 2310 GFLOPS Backward: 2536 GFLOPS, (1955 GFLOPS), (3606
// GFLOPS) y checksum 6.358978E+05     hy checksum 6.281680E+04 dx
// checksum 6.296622E+00    dhx checksum 2.289960E+05 dw checksum 5.397419E+07
...
// This function does all the work of setting up and running cuDNN's RNN
// functions with the given parameters. It also calculates performance results
// and checksums, printing them to the command line and saving them to
// result.txt for potential comparison to the files (golden_1.txt, etc).
template <typename T_ELEM>
void doTest(int seqLength, int numLayers, int hiddenSize, int inputSize,
            int miniBatch, float dropout, bool bidirectional,
            cudnnRNNMode_t mode, cudnnRNNAlgo_t algo) {
  FILE* fp = NULL;
  ...
  cudaErrCheck(
      cudaMalloc((void**)&x, seqLength * inputTensorSize * sizeof(T_ELEM)));
  cudaErrCheck(
      cudaMalloc((void**)&y, seqLength * outputTensorSize * sizeof(T_ELEM)));
  cudaErrCheck(
      cudaMalloc((void**)&dx, seqLength * inputTensorSize * sizeof(T_ELEM)));
  cudaErrCheck(
      cudaMalloc((void**)&dy, seqLength * outputTensorSize * sizeof(T_ELEM)));
  ...
  for (int i = 0; i < seqLength; i++) {
    ...
    cudnnErrCheck(cudnnSetTensorNdDescriptor(xDesc[i], getDataType<T_ELEM>(), 3,
                                             dimIn, strideIn));
    cudnnErrCheck(cudnnSetTensorNdDescriptor(dxDesc[i], getDataType<T_ELEM>(),
                                             3, dimIn, strideIn));

    cudnnErrCheck(cudnnSetTensorNdDescriptor(yDesc[i], getDataType<T_ELEM>(), 3,
                                             dimOut, strideOut));
    cudnnErrCheck(cudnnSetTensorNdDescriptor(dyDesc[i], getDataType<T_ELEM>(),
                                             3, dimOut, strideOut));
  }
  ...
  cudnnErrCheck(cudnnSetTensorNdDescriptor(hxDesc, getDataType<T_ELEM>(), 3,
                                           dimHidden, strideHidden));
  cudnnErrCheck(cudnnSetTensorNdDescriptor(cxDesc, getDataType<T_ELEM>(), 3,
                                           dimHidden, strideHidden));
  cudnnErrCheck(cudnnSetTensorNdDescriptor(hyDesc, getDataType<T_ELEM>(), 3,
                                           dimHidden, strideHidden));
  cudnnErrCheck(cudnnSetTensorNdDescriptor(cyDesc, getDataType<T_ELEM>(), 3,
                                           dimHidden, strideHidden));

  cudnnErrCheck(cudnnSetTensorNdDescriptor(dhxDesc, getDataType<T_ELEM>(), 3,
                                           dimHidden, strideHidden));
  cudnnErrCheck(cudnnSetTensorNdDescriptor(dcxDesc, getDataType<T_ELEM>(), 3,
                                           dimHidden, strideHidden));
  cudnnErrCheck(cudnnSetTensorNdDescriptor(dhyDesc, getDataType<T_ELEM>(), 3,
                                           dimHidden, strideHidden));
  cudnnErrCheck(cudnnSetTensorNdDescriptor(dcyDesc, getDataType<T_ELEM>(), 3,
                                           dimHidden, strideHidden));
  ...
  cudnnErrCheck(cudnnSetRNNDescriptor_v6(
      cudnnHandle, rnnDesc, hiddenSize, numLayers, dropoutDesc,
      CUDNN_LINEAR_INPUT,  // We can also skip the input matrix transformation
      bidirectional ? CUDNN_BIDIRECTIONAL : CUDNN_UNIDIRECTIONAL, mode,
  ...
  size_t weightsSize;
  cudnnErrCheck(cudnnGetRNNParamsSize(cudnnHandle, rnnDesc, xDesc[0],
                                      &weightsSize, getDataType<T_ELEM>()));
  ...
  cudnnErrCheck(cudnnSetFilterNdDescriptor(wDesc, getDataType<T_ELEM>(),
                                           CUDNN_TENSOR_NCHW, 3, dimW));
  cudnnErrCheck(cudnnSetFilterNdDescriptor(dwDesc, getDataType<T_ELEM>(),
                                           CUDNN_TENSOR_NCHW, 3, dimW));
  ...
  // Need for every pass
  cudnnErrCheck(cudnnGetRNNWorkspaceSize(cudnnHandle, rnnDesc, seqLength, xDesc,
                                         &workSize));
  // Only needed in training, shouldn't be touched between passes.
  cudnnErrCheck(cudnnGetRNNTrainingReserveSize(cudnnHandle, rnnDesc, seqLength,
                                               xDesc, &reserveSize));
  ...
  for (int layer = 0; layer < numLayers * (bidirectional ? 2 : 1); layer++) {
    for (int linLayerID = 0; linLayerID < numLinearLayers; linLayerID++) {
      ...
      cudnnErrCheck(cudnnGetRNNLinLayerMatrixParams(
          cudnnHandle, rnnDesc, layer, xDesc[0], wDesc, w, linLayerID,
      ...
      initGPUData<T_ELEM>(
          linLayerMat, filterDimA[0] * filterDimA[1] * filterDimA[2],
          1.0 / (filterDimA[0] * filterDimA[1] * filterDimA[2]));
      ...
      initGPUData<T_ELEM>(linLayerBias,
                          filterDimA[0] * filterDimA[1] * filterDimA[2], 1.0);
  ...
  if (algo == CUDNN_RNN_ALGO_PERSIST_DYNAMIC) {
    // Note: This step is expensive. Once completed the plan can be reused so
    // long as the descriptor
    //       minibatch or datatype don't change.
    cudnnErrCheck(cudnnCreatePersistentRNNPlan(
        rnnDesc, miniBatch, getDataType<T_ELEM>(), &rnnPlan));
  ...
  cudnnErrCheck(cudnnRNNForwardTraining(
      cudnnHandle, rnnDesc, seqLength, xDesc, x, hxDesc, hx, cxDesc, cx, wDesc,
      w, yDesc, y, hyDesc, hy, cyDesc, cy, workspace, workSize, reserveSpace,
      reserveSize));
  ...
  cudnnErrCheck(cudnnRNNBackwardData(
      cudnnHandle, rnnDesc, seqLength, yDesc, y, dyDesc, dy, dhyDesc, dhy,
      dcyDesc, dcy, wDesc, w, hxDesc, hx, cxDesc, cx, dxDesc, dx, dhxDesc, dhx,
      dcxDesc, dcx, workspace, workSize, reserveSpace, reserveSize));
  ...
  cudnnErrCheck(cudnnRNNBackwardWeights(
      cudnnHandle, rnnDesc, seqLength, xDesc, x, hxDesc, hx, yDesc, y,
  ...
  printf("(%3.0f GFLOPS), ", numMats * 2ull * (bidirectional ? 2 : 1) *
                                 hiddenSize * hiddenSize * seqLength *
                                 miniBatch * numLayers / (1e6 * timeBackward1));
  printf("(%3.0f GFLOPS)\n", numMats * 2ull * (bidirectional ? 2 : 1) *
                                 hiddenSize * hiddenSize * seqLength *
                                 miniBatch * numLayers / (1e6 * timeBackward2));

  // Save FLOPS to file
  fprintf(fp, "Forward: %3.0f GFLOPS\n",
          numMats * 2ull * (bidirectional ? 2 : 1) * hiddenSize * hiddenSize *
              seqLength * miniBatch * numLayers / (1e6 * timeForward));
  fprintf(fp, "Backward: %3.0f GFLOPS, ",
          numMats * 4ull * (bidirectional ? 2 : 1) * hiddenSize * hiddenSize *
              seqLength * miniBatch * numLayers /
              (1e6 * (timeBackward1 + timeBackward2)));
  ...
  if (true) {
    ...
    testOutputy =
        (T_ELEM*)malloc(seqLength * outputTensorSize * sizeof(T_ELEM));
    ...
    cudaErrCheck(cudaMemcpy(testOutputy, y,
                            seqLength * outputTensorSize * sizeof(T_ELEM),
    ...
    for (int m = 0; m < miniBatch; m++) {
      ...
      for (int j = 0; j < seqLength; j++) {
        for (int i = 0; i < hiddenSize * biDirScale; i++) {
          localSumi +=
              (double)testOutputy[j * miniBatch * hiddenSize * biDirScale +
  ...
  if (true) {
    ...
    testOutputdx =
        (T_ELEM*)malloc(seqLength * inputTensorSize * sizeof(T_ELEM));
    testOutputdhx = (T_ELEM*)malloc(hiddenTensorSize * sizeof(T_ELEM));
    testOutputdcx = (T_ELEM*)malloc(hiddenTensorSize * sizeof(T_ELEM));

    cudaErrCheck(cudaMemcpy(testOutputdx, dx,
                            seqLength * inputTensorSize * sizeof(T_ELEM),
                            cudaMemcpyDeviceToHost));
    if (dhx != NULL) {
      cudaErrCheck(cudaMemcpy(testOutputdhx, dhx,
                              hiddenTensorSize * sizeof(T_ELEM),
                              cudaMemcpyDeviceToHost));
    }
    if ((dcx != NULL) && (mode == CUDNN_LSTM)) {
      cudaErrCheck(cudaMemcpy(testOutputdcx, dcx,
                              hiddenTensorSize * sizeof(T_ELEM),
                              cudaMemcpyDeviceToHost));
    }
    ...
    for (int m = 0; m < miniBatch; m++) {
      ...
      for (int j = 0; j < seqLength; j++) {
        for (int i = 0; i < inputSize; i++) {
          localSumdx += (double)
              testOutputdx[j * miniBatch * inputSize + m * inputSize + i];
        }
      }
      ...
      for (int j = 0; j < numLayers * biDirScale; j++) {
        for (int i = 0; i < hiddenSize; i++) {
          localSumdhx += (double)
              testOutputdhx[j * hiddenSize * miniBatch + m * hiddenSize + i];
          if (mode == CUDNN_LSTM) {
            localSumdcx += (double)
                testOutputdcx[j * hiddenSize * miniBatch + m * hiddenSize + i];
          }
        }
      }
...
int parse_args(int argc, char* argv[], int& seqLength, int& numLayers,
               int& hiddenSize, int& inputSize, int& miniBatch, float& dropout,
               bool& bidirectional, cudnnRNNMode_t& mode, cudnnRNNAlgo_t& algo,
               cudnnDataType_t& dataType) {
  argc -= 1;
  argv++;
  while (argc) {
    if (argv[0][0] == '-') {
      if (strncmp(argv[0] + 1, "seqLength", strlen("seqLength")) == 0) {
      ...
      } else if (strncmp(argv[0] + 1, "hiddenSize", strlen("hiddenSize")) ==
                 0) {
        ...
        sscanf(p, "%f", &dropout);
      } else if (strncmp(argv[0] + 1, "bidirectional",
        ...
        printf(
            "  -bidirectional     : Switch to bidirectional instead of "
            "unidirectional RNN\n");
        printf("  -mode{0,1,2,3}     : Specify mode (ReLU, tanh, LSTM, GRU)\n");
        printf(
            "  -persistent{0,1,2} : Specify recurrence algorithm (standard, "
            "persist dynamic, persist static)\n");
        printf(
            "  -P{s,d,h}          : Specify data type precision (float, "
            "double, half)\n");
        printf("  -H                 : Display this help message\n");
        return 1;
      } else {
        printf(
            "Improper command line flag! See \"./RNN -H\" for proper usage\n");
        return 1;
      }
    } else {
...
// Checks whether the given parameters are supported by cuDNN, and prints them
// to the command line
int print_args(int seqLength, int numLayers, int hiddenSize, int inputSize,
               int miniBatch, float dropout, bool bidirectional,
               cudnnRNNMode_t mode, cudnnRNNAlgo_t algo,
               cudnnDataType_t dataType) {
  printf("seqLength  = %d\nnumLayers  = %d\nhiddenSize = %d\n", seqLength,
  ...
  if (dataType == CUDNN_DATA_FLOAT) {
    ...
    if (algo == CUDNN_RNN_ALGO_PERSIST_STATIC) {
      printf(
          "!!! ERROR: Double precision is disabled for PERSIST_STATIC "
          "algorithm\n");
      return 1;
    }
...
int main(int argc, char* argv[]) {
  ...
  bool error =
      parse_args(argc, argv, seqLength, numLayers, hiddenSize, inputSize,
                 miniBatch, dropout, bidirectional, mode, algo, dataType);
  ...
  error = print_args(seqLength, numLayers, hiddenSize, inputSize, miniBatch,
                     dropout, bidirectional, mode, algo, dataType);
  ...
  if (dataType == CUDNN_DATA_FLOAT) {
    doTest<float>(seqLength, numLayers, hiddenSize, inputSize, miniBatch,
                  dropout, bidirectional, mode, algo);
  } else if (dataType == CUDNN_DATA_DOUBLE) {
    doTest<double>(seqLength, numLayers, hiddenSize, inputSize, miniBatch,
                   dropout, bidirectional, mode, algo);
  } else if (dataType == CUDNN_DATA_HALF) {
    doTest<half1>(seqLength, numLayers, hiddenSize, inputSize, miniBatch,
                  dropout, bidirectional, mode, algo);
  }
